...
const PBULICFUNC = {
    ...
    for (let i = 0; i < param.length; i++) {
      ...
      if (param[i].children_list && param[i].children_list.length) {
        for (let j = 0; j < param[i].children_list.length; j++) {
          ...
          if (param[i].children_list[j] && param[i].children_list[j].children_list) {
            for (let k = 0; k < param[i].children_list[j].children_list.length; k++) {
              map.set(param[i].children_list[j].children_list[k].menu_url, param[i].children_list[j].children_list[k])
            }
          }
        }
      }
  ...
  getTreeData(list, selectedID) {
    ...
    for (let i = 0; i < list.length; i++) {
      if (selectedID && list[i].orgID === selectedID) {
        list[i].selected = true
      }
      if (!selectedID) {
        list[0].selected = true
      }
      if (list[i] && list[i].parentID) {
        for (let j = 0; j < list.length; j++) {
          if (list[i].parentID === list[j].orgID) {
